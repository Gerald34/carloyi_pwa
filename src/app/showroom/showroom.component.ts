...
export class ShowroomComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.name = this.userDataInfo.name[0].toUpperCase() + this.userDataInfo.name.substr(1);
    this.surname = this.userDataInfo.lastName[0].toUpperCase() + this.userDataInfo.lastName.substr(1);
  ...
  getOfferDetails(itemID) {
    this.showroomService.offerDetails(itemID).subscribe((data: any) => {
   ...
   * @returns {Subscription}
   */
  public sendRequest(requestID, userID, userEmail, userExtras) {

    const requestData = { cid: requestID, uid: userID, email: userEmail, extras: userExtras };
    if (this.userDataInfo.status === 1) {
      return this.showroomService.placeRequest(requestData).subscribe((data: any) => {
        if (data.successCode === 200) {
          this.toastr.success('Request was successfully sent', 'Great!!');
        } else if (data.code === -1) {
          this.toastr.info(data.error, 'Oops!');
        } else {
          this.toastr.info(data.errorMessage, 'Oops!');
        }
      });
    } else {
      this.toastr.info('Attention', 'Please provide us with important information before we proceed');
    }
  }
  ...
  public getUpdateData(
    ...
    if (this.agreement === 'on') {
      ...
      this.toastr.warning('Please consent to the terms of use', 'Notice!');
    }
  }
  ...
  private _updateProfile(updateProfileData: any) {
    return this.showroomService.updateUserInformation(updateProfileData).subscribe((data: any) => {
      if (typeof data.successCode !== 'undefined') {
        ...
        this.toastr.warning(this.response, 'Sorry');
      }
    });
  }
  ...
  toggleEditable(event) {
    if (event.target.checked) { this.agreement = 'on'; }
  }
